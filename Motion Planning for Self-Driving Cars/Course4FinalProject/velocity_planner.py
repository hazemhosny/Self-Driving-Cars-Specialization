...
class VelocityPlanner:
    ...
    def decelerate_profile(self, path, start_speed): 
        ...
        if brake_distance + decel_distance + stop_line_buffer > path_length:
            speeds = []
            vf = 0.0
            print(" --------------------------- harder deceleration is required --------------------------- ") #HH
            ...
            for i in reversed(range(stop_index)):
                dist = np.linalg.norm([path[0][i+1] - path[0][i], 
                                       path[1][i+1] - path[1][i]])
                vi = calc_final_speed(vf, self._a_max, dist)      #edited by Hazem Hosny from -ve accel to +ve
                ...
                speeds.insert(0, vi)
                vf = vi
            print("speed list : ", speeds) #added by Hazem Hosny
            print(" --------------------------------------------------------------------------------------- ") #HH
